...
def generate_quadtree(elements, node_factory=QuadTreeNodeFactory()):
    ...
    # Construct quadtree
    for el in elements:
        q_tree.add_element(el, np.array([[-12, 12], [12, 12], [12, -12], [-12, -12]]))
    # Traverse quadtree
...
def visualize_quadtree(r_points, intermediate_nodes, leaf_nodes):
    ax = plt.gca()
    ax.scatter(r_points[:, 0], r_points[:, 1])
    ...
    for l in leaf_nodes:
        b = l.boundary
        c_point = b.center_point()
        ax.scatter(c_point[0], c_point[1], color='b', linewidths=5, alpha=0.2)
        # Draw leaf boundary
        anchor = b.points[3]
        rect_boundary = patches.Rectangle(anchor, b.width(), b.height(), edgecolor='blue', facecolor='none', alpha=0.6)
        ax.add_patch(rect_boundary)
    plt.show()
    # Graphical view
